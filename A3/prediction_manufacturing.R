...
data <- data %>% filter(ind2 >= 26 & ind2 <= 33)
...
write_csv(cm3, "cm3_manufacturing.csv")
...
write_csv(summary_results, "summary_results_manufacturing.csv")
model_names <- c("Logit X1", "Logit X3",
                 "Logit LASSO","RF probability")
summary_results <- summary_results %>%
  filter(rownames(.) %in% c("X1", "X3", "LASSO", "rf_p"))
rownames(summary_results) <- model_names
write_csv(summary_results, "sum_manufacturing.csv")
