...
jitter_edu <- ggplot(dt,aes(factor(educ), w, color = "#440154" )) + 
  geom_boxplot(alpha = 0.1, frame = FALSE) + 
  geom_jitter(height = 0, width = 0.1, alpha = 0.2) +
  scale_color_viridis(option = "D", discrete = TRUE) +
  scale_fill_viridis(option = "D", discrete = TRUE) +
...
jitter_class <- ggplot(dt,aes(factor(class), w, color = "#440154" )) + 
  ...
  scale_fill_viridis(option = "D", discrete = TRUE) +
  theme_bw()+
  theme(legend.position = "none", axis.text.x = element_text(angle= 45, vjust=.5), panel.border = element_blank()) +
...
boxplot_edu_gender <- ggplot(dt, aes(x = factor(educ), y = w, fill = factor(gender), color=factor(gender))) +
...
boxplot_edu_married <- ggplot(dt, aes(x = factor(educ), y = w, fill = factor(gender), color=factor(gender))) +
  geom_boxplot(alpha=0.6, na.rm=T, outlier.shape = NA, width = 0.8) +
  stat_boxplot(geom = "errorbar", width = 0.8, size = 0.3, na.rm=T)+
  scale_color_viridis(option = "D", discrete = TRUE) +
  scale_fill_viridis(option = "D", discrete = TRUE) +
...
boxplot_class_gender <- ggplot(dt, aes(x = factor(class), y = w, fill = factor(gender), color=factor(gender))) +
  ...
  theme_bw() +
  theme(legend.position = "top", axis.text.x = element_text(angle= 45, vjust=.5), panel.border = element_blank())+
...
The purpose of this assignment is to build four predictive models using linear regression. The source of data for this purpose is CPS-earning data set which can be found [here](https://osf.io/g8p9j/). The chosen occupation for this project is _Human Resource worker_ with the census occupation code of _630_. 
...
m3 <- as.formula(w ~ ed_associate + ed_BA + ed_MA + ed_Profess + ed_PhD + age + agesq + female + privt_sec + child + marital_status)

m4 <- as.formula(w ~ ed_associate + ed_BA + ed_MA + ed_Profess + ed_PhD + age + agesq + female + privt_sec + marital_status + ed_associate *female  + female * ed_BA +  + female * ed_MA + female * ed_Profess + female * ed_PhD + female * age + female * agesq + female * privt_sec + female * child)
...
etable( 'M1' = reg1 , 'M2' = reg2 , 'M3' = reg3 , 'M4' = reg4 , 
        dict = variable_names, title = 'Regression Models for Earning per hour',
        fitstat = c('aic','bic','rmse','r2','n','k') ) %>% kbl(booktabs = T) %>% kable_styling(full_width = T)
...
grid.arrange( grobs = list(hist_w, hist_w_ln, hist_w_f, hist_w_m, hist_age_f, hist_age_m), ncol = 2)
...
grid.arrange( grobs = list(jitter_gender, jitter_child, jitter_edu, jitter_married, jitter_class ), ncol = 2)
...
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, fig.height=10, fig.width=8}

grid.arrange( grobs = list(boxplot_edu_gender, boxplot_married_gender, boxplot_edu_married, boxplot_child_gender, boxplot_class_gender), ncol = 2)
